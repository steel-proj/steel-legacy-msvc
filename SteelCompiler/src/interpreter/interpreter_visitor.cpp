		...
		case TT_ADD:
			if (left->is_string() && right->is_string()) {
				...
				if (left->is_float() || right->is_float()) {
					...
					expression_result = std::make_shared<runtime_value>(data_type(DT_INT), std::to_string(left->as_int() + right->as_int()));
				}
			}
			else {
...
void interpreter_visitor::visit(std::shared_ptr<unary_expression> expr) {
	...
	case TT_INCREMENT: {
		...
		if (var->is_int()) {
			var = std::make_shared<runtime_value>(data_type(DT_INT), std::to_string(var->as_int() + 1));
		}
	...
	case TT_SUBTRACT: {
		if (operand->is_int()) {
			expression_result = std::make_shared<runtime_value>(data_type(DT_INT), std::to_string(-operand->as_int()));
		}
