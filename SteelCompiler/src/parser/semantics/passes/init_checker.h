...
public:
	init_checker(std::shared_ptr<symbol_table> sym_table)
		: sym_table(sym_table) {
	}

	void visit(std::shared_ptr<class type_decleration> decl) override;
	void visit(std::shared_ptr<class constructor_decleration> ctor_decl) override;
	void visit(std::shared_ptr<class function_decleration> func_decl) override;
...
private:
	...
	std::unordered_set<std::shared_ptr<variable_decleration>> initialized;
