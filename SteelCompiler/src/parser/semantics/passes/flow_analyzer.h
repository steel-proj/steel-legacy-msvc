#pragma once

#include <string>
#include <map>
#include <unordered_set>

#include "../../ast/ast_visitor.h"
#include "../../compilation_pass.h"
#include "../../symbolics/symbol_table.h"

class flow_analyzer : public ast_visitor, public compilation_pass {
public:
	flow_analyzer(std::shared_ptr<symbol_table> sym_table)
		: sym_table(sym_table), current_function(nullptr), current_constructor(nullptr), in_loop(false), current_returns(false) {
	}

	void visit(std::shared_ptr<class function_decleration> func) override;
	void visit(std::shared_ptr<class constructor_decleration> constructor) override;
	void visit(std::shared_ptr<class constructor_call> ctor_call) override;
	void visit(std::shared_ptr<class block_statement> block) override;
	void visit(std::shared_ptr<class return_statement> ret_stmt) override;
	void visit(std::shared_ptr<class if_statement> if_stmt) override;
	//void visit(std::shared_ptr<class for_loop> for_loop) override;
	//void visit(std::shared_ptr<class while_loop> while_loop) override;

private:
	std::shared_ptr<symbol_table> sym_table;

	std::shared_ptr<class function_decleration> current_function;
	std::shared_ptr<class constructor_decleration> current_constructor;
	bool in_loop;

	bool current_returns;
};