...
void flow_analyzer::visit(std::shared_ptr<function_decleration> func) {
...
void flow_analyzer::visit(std::shared_ptr<constructor_decleration> constructor) {
...
void flow_analyzer::visit(std::shared_ptr<constructor_call> ctor_call) {
	// if the constructor call decleration is the same as we are in,
	// it would result in a stack overflow
	if (current_constructor && ctor_call->decleration == current_constructor) {
...
void flow_analyzer::visit(std::shared_ptr<return_statement> ret_stmt) {
	...
	if (ret_stmt->value) {
		...
		// ensure types match
		if (*func_type != *ret_stmt->value->type()) {
