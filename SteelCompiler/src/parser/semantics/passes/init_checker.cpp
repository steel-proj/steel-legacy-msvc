...
void init_checker::visit(std::shared_ptr<type_decleration> decl) {
...
void init_checker::visit(std::shared_ptr<constructor_decleration> ctor_decl) {
...
void init_checker::visit(std::shared_ptr<function_decleration> func) {
	// mark all parameters as initialized
...
void init_checker::visit(std::shared_ptr<variable_decleration> var) {
...
void init_checker::visit(std::shared_ptr<assignment_expression> expr) {
	...
	if (auto id = std::dynamic_pointer_cast<identifier_expression>(expr->left)) {
		initialized.insert(id->decleration);
		id->decleration->initialized = true;
	}
}
void init_checker::visit(std::shared_ptr<identifier_expression> expr) {
	...
	if (initialized.find(expr->decleration) == initialized.end()) {
...
void init_checker::visit(std::shared_ptr<if_statement> if_stmt) {
	...
	std::unordered_set<std::shared_ptr<variable_decleration>> intersection;
