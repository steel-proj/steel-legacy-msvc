...
ast_ptr parser::parse_return_statement() {
	...
	// if no semi colon is present after return, it is an expression
	if (match(TT_SEMICOLON)) {
...
ast_ptr parser::parse_primary_expression() {
	...
	if (match(TT_IDENTIFIER)) {
		...
		expr = make_ast<literal>(literal_token, DT_INT, literal_token.value);
	}
...
bool parser::match_typename() {
	if (match(10, TT_SMALL, TT_INT, TT_LARGE, TT_FLOAT, TT_DOUBLE, TT_CHAR, TT_STRING, TT_BYTE, TT_BOOL, TT_VOID)) {
		return true;
	}
